ushort computeDistance( __global const uchar* target, int target_row, int target_col, __global const uchar* source, 
                      int source_row, int source_col, const uint4 imageSize){
	long distance = 0;
	long weight_sum = 0;
	int penalty = 655350;
	int radius = 2;
 ...
 __kernel void jumpFloodingCopy(__global const char* source, __global const char* target, __global ushort* oldNNF,  __global ushort* oldDistance, 
                          const uint4 imageSize, __global const uchar* mask)
{
	...
	for(int iter = 0; iter < iternum; iter++){
		...
		while(window_radius > 0){
			...
			if(mask[row * imageSize.y + col] == 0)
			{
				...
				dis[k] = computeDistance(target, row, col, source, row, col, imageSize);
				rows[k] = row;
				cols[k] = col;
				k++;
				...
				for(int i = 0; i < 8; i++){
					...
					dis[k] = computeDistance(target, row, col, source, ROW[i], COL[i], imageSize);
					rows[k] = ROW[i];
					cols[k] = COL[i];
					k++;
					...
					for(int j = 0; j < knn; j++){	
						...
						dis[k] = computeDistance(target, row, col, source, currentRow, currentCol, imageSize);
						rows[k] = currentRow;
						cols[k] = currentCol;
						k++;
					}
